...
def change_temp_figure(data):
    ...
    fig1 = df.plot.line(figsize=(7, 2), title="Temperature", legend="").get_figure()
...
def change_pressure_figure(data):
    ...
    df = pd.DataFrame(data=my_dict)
    fig2 = df.plot.line(figsize=(7, 2), title="Pressure", legend="").get_figure()
...
def change_acceleration_figure(data):
    ...
    df = pd.DataFrame(data=my_dict)
    fig3 = df.plot.line(figsize=(7, 2), title="Acceleration", legend="").get_figure()
